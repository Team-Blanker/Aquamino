...
local cbl={}
local function checkBeside(x,y,id)--确认附近的“鱼”
...
for i=1,128 do
    ...
    --bg.arrowList[i].animAngle=bg.arrowList[i].angle
...
local function setAttach(x,y,id,v,angle)
    for i=1,#oList,2 do
...
function bg.update(dt)
    ...
    if bg.updateT>=tick then bg.updateT=bg.updateT-tick
        for id=1,#bg.arrowList do
            ...
            --arw.animAngle=arw.animAngle*(1-24*tick)+arw.angle*24*tick--每一帧线性插值

            f=arw.state==1
            if f then
                setAttach(arw.attach.x,arw.attach.y,id,nil,arw.angle)
            end

            arw.attach.x,arw.attach.y=floor((arw.x-1*attach*cos(arw.angle))/attach+.5),floor((arw.y-1*attach*sin(arw.angle))/attach+.5)
            ...
            if f then
                ...
                if arw.guideT<=0 then
                    ...
                    setAttach(arw.attach.x,arw.attach.y,id,true,arw.angle)
                end
            else
            ...
            if arw.follow.angle>0 then
                arw.angle=arw.angle+min(12*tick,arw.follow.angle)
                arw.follow.angle=max(arw.follow.angle-12*tick,0)
            elseif arw.follow.angle<0 then
                arw.angle=arw.angle-min(12*tick,-arw.follow.angle)
                arw.follow.angle=min(arw.follow.angle+12*tick,0)
            else
