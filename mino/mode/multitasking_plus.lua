...
function rule.init(P,mino)
    ...
    scene.BG=require('BG/sqdance') --scene.BG.init()
    ...
    P[1].FDelay=1e99
    P[1].posX=-600
    P[2]=myTable.copy(P[1]) P[2].posX=0 mino.setStackerOperate(2)
    P[3]=myTable.copy(P[1]) P[3].posX=600 mino.setStackerOperate(3)
    ...
    mino.fieldScale=min(mino.fieldScale,.75)
end
function rule.onLineClear(player,mino)
    local his=player.history
    player.totalLine=player.totalLine+his.line
    while player.totalLine>=player.speedLv*2 do
        local win=true
        for k,v in pairs(mino.player) do
            if v.totalLine<20 then win=false break end
        end
        if win then
            for k,v in pairs(mino.player) do mino.win(v) end return
        end
        if player.speedLv<11 then
...
function rule.underFieldDraw(player)
    ...
    gc.printf(""..min(player.speedLv,10)*2,font.JB_B,-player.w*18-110,36,2048,'center',0,.5,.5,1024,font.height.JB_B/2)
    gc.printf("Level "..min(player.speedLv,10),font.JB_B,-player.w*18-28,288,2048,'right',0,0.25,0.25,2048,font.height.JB_B/2)
    gc.setLineWidth(7)
