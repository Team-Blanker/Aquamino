...
local e1cl=gc.newCanvas(160,320) local e1cr=gc.newCanvas(160,320)
local e1cw=gc.newCanvas(320,320)
gc.setLineWidth(20) gc.setColor(1,1,1)
gc.setCanvas(e1cl) gc.circle('line',160,160,150,4) gc.rectangle('fill',0,0,160,320)
gc.setCanvas(e1cr) gc.circle('line',0,160,150,4)
gc.setCanvas(e1cw) gc.circle('line',160,160,150,4)
gc.setCanvas()
function anim.enter1(intime,keeptime,outtime)
    ...
    if scene.swapT>0 then
        ...
        if scene.swapT>keeptime then
            ...
            gc.setColor(1,1,1)
            gc.draw(e1cl,-960+960*rect_w,0,0,1,1,160,160)
            gc.draw(e1cr, 960-960*rect_w,0,0,1,1,0,160)
        else
            gc.rectangle('fill',-960,-540,1920,1080)
            gc.setColor(1,1,1)
            gc.draw(e1cw,0,0,0,1,1,160,160)
        end
    else
