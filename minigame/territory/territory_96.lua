...
local function preSolve(fa,fb,coll)
    if (war.onCollide[fa] or war.onCollide[fb]) and not myTable.include(contactList,coll) then table.insert(contactList,coll) end
end
...
function war.init()
    ...
    war.team={
        ...
        rCool={0,0,0,0},rCoolT=1/128--子弹发射速度频率为1/rCoolT，cool意为冷却
    }
    war.teamBelong={}
    war.bulletLimit=2048
    ...
    for t=1,4 do--创建控制区边界以及障碍物
        ...
        war.ctrl.cmd[t]={}
        war.ctrl.cmd[t].mtp={
            body=LP.newBody(war.world,m+(left and -1 or 1)*40,n+10.5*16,'static'),
            shape=LP.newRectangleShape(13*16,32),
        }
        ...
        war.ctrl.cmd[t].rel={
            body=LP.newBody(war.world,m-(left and -1 or 1)*104,n+10.5*16,'static'),
            shape=LP.newRectangleShape(5*16,32),
        }
        ...
        for i=1,#obsList do
            war.ctrl.obs[#war.ctrl.obs+1]={
                body=LP.newBody(war.world,m+80*(left and -1 or 1)*obsList[i][1],n-80*obsList[i][2],'static'),
    ...
    for i=1,96 do--创建领土并设置分类与遮罩
        war.field[i]={}
        for j=1,96 do
            war.field[i][j]={
                body=LP.newBody(war.world,8*(i-48.5),8*(j-48.5),'static'),
                shape=LP.newRectangleShape(8,8),
            }
...
function war.update(dt)
    if war.sim then war.updateTimer=war.updateTimer+dt
        while war.updateTimer>=1/128 do war.gameUpdate(1/128) war.updateTimer=war.updateTimer-1/128 end
    end
end
local vx,vy
...
function war.draw()
    ...
    for i=1,#war.field do
        for j=1,#war.field[i] do
            ...
            draw(fieldCanvas,8*(i-49),8*(j-49))
        end
    end
    ...
    for i=1,#war.ctrl.cmd do
        ...
        setColor(1,0,0,.5)
        printf('FIRE',font.JB,war.ctrl.cmd[i].rel.body:getX(),war.ctrl.cmd[i].rel.body:getY()-32,1280,'center',0,.25,.25,640,font.height.JB/2)
        ...
        setColor(0,1,0,.5)
        printf('x2',font.JB,war.ctrl.cmd[i].mtp.body:getX(),war.ctrl.cmd[i].mtp.body:getY()-32,1280,'center',0,.25,.25,640,font.height.JB/2)
    end
    gc.setColor(1,1,1,.8)
    ...
    for i=1,4 do
        ...
        if war.team.bulletR[i]>0 then gc.setColor(c[1],c[2],c[3],.5)
            printf(""..war.team.bulletR[i],font.JB_B,m,n,1280,'center',0,.75,.75,640,font.height.JB_B/2)
        else
            if war.team.bulletS[i]==war.bulletLimit then gc.setColor(c[1],c[2],c[3],.5) else gc.setColor(.75,.75,.75,.5) end
            printf(""..war.team.bulletS[i],font.JB_B,m,n,1280,'center',0,.75,.75,640,font.height.JB_B/2)
        end
    end
    ...
    setColor(1,1,1,.3)
    printf(string.format("%02d:%02d",war.time/60,war.time%60),font.JB_B,0,0,10000,'center',0,1,1,5000,font.height.JB_B/2)
    setColor(1,1,1,2+supplyT-supplyTLimit)--显示新球已加入
    printf("New balls added.",font.JB_B,0,0,10000,'center',0,.5,.5,5000,font.height.JB_B/2)
